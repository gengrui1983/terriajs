...
function reviseLegendHelper(regionMapping) {
    console.log("regionMapping._tableStructure.activeItem.length", regionMapping._tableStructure.activeItems.length);
    ...
    regionMapping._legendHelper.generateSecondaryItem(activeColumn, regionMapping._tableStructure.items);
...
function createNewRegionImageryLayer(regionMapping, layerIndex, regionIndices, globeOrMap) {
    ...
    if (!defined(regionIndices)) {
        failedMatches = [];
        ambiguousMatches = [];
        regionIndices = calculateRegionIndices(regionMapping, globeOrMap.terria.clock.currentTime, failedMatches, ambiguousMatches);
    ...
    var alphaFunction = regionDetail.regionProvider.getAlphaLookupFunc(
        regionValues,
        legendHelper.getAlphaFromValue.bind(legendHelper)
    );
    ...
    case "MVT":
        ...
        layer = ImageryLayerCatalogItem.enableLayer(catalogItem, regionImageryProvider, catalogItem.opacity, layerIndex, globeOrMap);
        break;
            ...
            layer.on('tileload', function(evt) {
                ...
                */
                ...
                if (ctx) {
                    ...
                    imgd = ImageryProviderHooks.recolorImage(imgd, colorFunction, alphaFunction);
