...
var defaultMinAlpha = 0.7;
var defaultMaxAlpha = 1.0;
var defaultNumber = 200;
const MIN_ALPHA = 0.7;
...
var LegendHelper = function(tableColumn, tableStyle, regionProvider, name) {
    ...
    // This is for visualising the regions by some other columns.
    this._secondaryVariable = undefined;
};
...
function generateLegend(legendHelper) {
    ...
    if (!defined(legendHelper.tableColumn) || !defined(legendHelper.tableColumn.values)) {
        ...
        legendHelper._alphaGradient = buildAlphaGradient(defaultMinAlpha, defaultMaxAlpha, defaultNumber);
        legendHelper._binColors = buildBinColors(legendHelper, colorBins);
        legendHelper._binAlphas = buildBinAlphas(legendHelper, colorBins);
...
/**
 * Generate the secondary item by the activeItem
 */
LegendHelper.prototype.generateSecondaryItem = function (activeItem, items) {
    var i;
    for(i = 0; i < items.length; i++) {
        if(defined(activeItem) && items[i].id === activeItem.id + "_std") {
            this._secondaryVariable = items[i];
            break;
        }
    }
}
...
LegendHelper.prototype.getAlphaFromValue = function(value) {
    ...
    var i = 0;
    while (i < this._binAlphas.length - 1 && value > this._binAlphas[i].upperBound) {
        i++;
    }
    if (!defined(this._binAlphas[i])) { // is this actually possible given the checks above?
        console.log('Bad bin ' + i);
        return 0;
    }
    return this._binAlphas[i].alpha;
};
...
function buildAlphaGradient(minAlpha, maxAlpha, counts) {
    if (minAlpha > maxAlpha || minAlpha < MIN_ALPHA) {
        return;
    }

    var step = (maxAlpha - minAlpha) / counts;
    var tmpAlpha = minAlpha;
    var results = [];
    for(var i = 0; i < counts; i++) {
        tmpAlpha += step;
        results.push(tmpAlpha);
    }
...
function getAlphaFromAlphaGradient(alphaGradient, fractionalPosition) {
    var alphaIndex = Math.floor(fractionalPosition * (alphaGradient.length - 1));
    return alphaGradient[alphaIndex];
}
...
function buildBinAlphas(legendHelper, colorBins) {
    var tableColumn = defined(legendHelper._secondaryVariable) ? legendHelper._secondaryVariable : legendHelper.tableColumn;
    var tableColumnStyle = legendHelper.tableColumnStyle;
    var alphaGradient = legendHelper._alphaGradient;
    ...
    if (method === 'quantile') {
        ...
        for (i = 0; i < binCount; i++) {
            binAlphas.push({
                ...
                alpha: getAlphaFromAlphaGradient(alphaGradient, i / (binCount - 1))
            });
        }
